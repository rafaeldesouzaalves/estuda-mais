/* ================================================================
   ...
   v3 — mesma arquitetura modular da v2 (estado, dados simulados,
   renderizadores de página, API pública), com as novas features
   acrescentadas nas seções 16+.
   ================================================================ */
...
const ANNOUNCEMENTS = [
  { author:'Profa. Helena', subject:'Matemática', text:'A prova de sexta-feira abordará os capítulos 4 e 5. Tragam calculadora.', time:'2h atrás' },
  { author:'Prof. Diego', subject:'Português', text:'Entrega da redação prorrogada para segunda-feira.', time:'1 dia atrás' },
];
...
/* ---- repetição espaçada: cada flashcard tem intervalo e data da próxima revisão ---- */
let FLASHCARDS = {}; // populado por buildFlashcards() na primeira visita de cada matéria

/* ---- banco de questões para o gerador de quiz de recordação ativa ---- */
const QUIZ_BANK = {
...
/* ---- histórico de sessões de foco (preenchido ao concluir um pomodoro/cronômetro) ---- */
let SESSIONS = [
  { subject:'cie', minutes:25, when:'Ontem, 19:40' },
  { subject:'mat', minutes:25, when:'Ontem, 16:10' },
];

/* ---- mapa de calor de estudo: 12 semanas simuladas (0-4 = intensidade) ---- */
const HEATMAP_WEEKS = 14;
const STUDY_HEATMAP = Array.from({length: HEATMAP_WEEKS*7}, (_, i) => {
  // últimos dias tendem a ter mais atividade para refletir o streak atual
...
(function renderLoginHeat(){
  ...
  el.innerHTML = Array.from({length:64}, ()=>{
    ...
    return `<div class="lh-cell" style="background:${lvl===0?'':'rgba(216,195,76,'+(lvl*0.24)+')'}"></div>`;
  }).join('');
})();

/* ================================================================
   5. NAVEGAÇÃO — itens diferem por perfil
   (expandida na seção 16 com as novas páginas)
   ================================================================ */
const NAV_BY_ROLE = {
  aluno: [
    { group:'Estudo', items:[
      ...
      {id:'eisenhower', label:'Matriz Eisenhower', ic:'grid-2x2'},
    ...
    { group:'Sistema', items:[
      {id:'avisos', label:'Mural de avisos', ic:'megaphone', badge:2},
  ...
  professor: [
    ...
    { group:'Avaliação', items:[
      ...
      {id:'graficos', label:'Desempenho da turma', ic:'bar-chart-3'},
      {id:'auditoria', label:'Auditoria', ic:'clipboard-list'},
      {id:'avisos', label:'Mural de avisos', ic:'megaphone'},
    ]},
    { group:'Sistema', items:[ {id:'config', label:'Configurações', ic:'settings'} ]},
  ],
  responsavel: [
...
function buildNav(){
  ...
  groups.forEach(g=>{
    navEl.innerHTML += `<div class="nav-group-label">${g.group}</div>`;
    g.items.forEach(it=>{
      navEl.innerHTML += `<a class="nav-link" data-page="${it.id}" href="#">${icon(it.ic)}<span>${it.label}</span>${it.badge?`<span class="badge">${it.badge}</span>`:''}</a>`;
...
function goTo(page, extra){
  state.page = page;
  $('#topbar-title').textContent = PAGE_TITLES[page] || 'Estude Mais';
...
/* ================================================================
   ...
   (RENDERERS ganha novas entradas na seção 16)
   ================================================================ */
const RENDERERS = {
  inicio: renderInicio,
...
function renderInicio(){
  ...
  return `
    ...
    <div class="card reveal">${taskList}</div>
  ...
  return `
    ...
    <div class="card subject-detail reveal">
      ...
      <div id="tab-anexos" class="tab-pane hidden">
        ${canUpload ? `<div class="upload-zone" id="upload-zone">
          ${icon('upload-cloud')}
...
/* material agora registra "leitura" simulada quando o aluno clica em baixar,
   alimentando o Analytics de Leitura do professor (seção 16.6) */
...
function buildFlashcards(s){
  ...
  const now = Date.now();
  // ease factor inicial 2.5 (padrão do SM-2), reps = nº de revisões corretas seguidas
...
/* SM-2 simplificado: em caso de acerto, aumenta reps e recalcula o intervalo
   multiplicando pelo "ease factor"; em caso de erro, reps zera e o intervalo
   volta a 1 dia. Isso é puramente matemático — sem XP, pontos ou prêmios,
   conforme pedido para o módulo de estudo profundo. */
...
function renderGraficos(){
  ...
  return `
    ...
    <div class="section-title"><h3>Metas da semana</h3></div>
    <div class="grid-3">
      <div class="card goal-card reveal"><div class="g-ic">${icon('target')}</div><div><b class="mono" style="font-size:1.05rem;">10.5h</b><br><span style="font-size:.73rem;color:var(--ink-faint)">de 12h planejadas</span></div></div>
      <div class="card goal-card reveal"><div class="g-ic">${icon('check-circle-2')}</div><div><b class="mono" style="font-size:1.05rem;">18</b><br><span style="font-size:.73rem;color:var(--ink-faint)">tarefas concluídas</span></div></div>
      <div class="card goal-card reveal"><div class="g-ic">${icon('trending-up')}</div><div><b class="mono" style="font-size:1.05rem;">+12%</b><br><span style="font-size:.73rem;color:var(--ink-faint)">vs. semana anterior</span></div></div>
    </div>
...
function renderAvisos(){
  ...
  // ordem cronológica estrita, mais recente primeiro; cada item pode ser marcado como "Lido e Ciente"
  const sorted = [...ANNOUNCEMENTS].sort((a,b)=> (b.ts||0)-(a.ts||0));
  const list = sorted.map((a,i)=>`
    <div class="mural-item">
      <div class="mu-ic">${icon('megaphone')}</div>
      <div style="flex:1;">
        <div style="font-size:.83rem;"><b>${a.author}</b> <span style="color:var(--ink-faint); font-weight:400;">· ${a.subject} · ${a.time}</span></div>
...
/* ================================================================
   16. DADOS SIMULADOS DAS NOVAS FUNCIONALIDADES
   ================================================================ */

/* 16.1 — Carga horária estática declarada pelo professor (horas por atividade) */
const WORKLOAD = {
...
// meta semanal recomendada por matéria, usada para calcular o % de saturação
const WORKLOAD_TARGET_WEEKLY = 6; // horas/semana consideradas saudáveis por matéria

/* 16.2 — Notas Cornell por matéria (dicas | notas | resumo) */
const CORNELL_DATA = {};
...
/* 16.3 — Analytics de leitura de material (professor): % da turma que abriu/leu até o fim */
...
/* 16.4 — Dispersão horas x nota (relatório de consistência dos pais) */
const SCATTER_DATA = [
...
/* 16.5 — Auditoria de faltas e entregas (livro de ocorrências digital) */
let AUDIT_LOG = [
...
/* 16.6 — Time-Tracking avançado: cronômetro de tempo líquido por matéria */
const timeTracker = {
  running:false, subject:'mat', startedAt:null, elapsed:0, interval:null,
  log: [ // horas líquidas acumuladas nesta semana, por matéria
...
/* 16.7 — Modo Foco rígido: contador de tentativas de distração (troca de aba) */
const focusLock = { active:false, distractions:0, seconds:0, interval:null };

/* ================================================================
   17. FUNÇÕES CONSTRUTORAS (builders) DAS NOVAS FEATURES
   ================================================================ */

/* ---- 17.1 Matriz de Eisenhower automatizada ----
   Classifica as TASKS em 4 quadrantes com base em dois sinais que, no app
   real, viriam dos professores: "prazo próximo" (urgência) e "peso da
   atividade" (importância — provas/trabalhos valem mais que avisos). */
...
/* ---- 17.2 Time-Tracking: view completa do cronômetro ---- */
...
/* ---- 17.3 Notas Cornell (dicas | anotações | resumo) ---- */
...
/* ---- 17.4 Analytics de leitura (professor) ---- */
...
/* ---- 17.5 Gráfico de dispersão (horas × nota) ---- */
...
/* ---- 17.6 Índice de procrastinação ----
   Mede, das tarefas concluídas, quantas foram feitas nas últimas 24h antes
   do prazo (em vez de distribuídas ao longo do tempo disponível). */
...
function buildProcrastinationIndex(){
  const idx = computeProcrastinationIndex() || 62; // valor ilustrativo quando há poucas amostras
...
/* ---- 17.7 Planejador de carga horária estática (professor) + saturação ---- */
...
/* ---- 17.8 Auditoria de faltas e entregas ---- */
...
/* ---- 17.9 Diagnóstico BNCC (competências específicas por avaliação) ---- */
...
function attachPageEvents(page, extra){
  ...
  if(page==='inicio'){
    animateRing('#daily-ring', TASKS.filter(t=>t.done).length/TASKS.length);
    $all('.task-check').forEach(chk=> chk.addEventListener('click', ()=>{
      const id = +chk.dataset.id;
      const t = TASKS.find(x=>x.id===id);
      t.done = !t.done; t.status = t.done ? 'concluida':'andamento';
      t.completedAt = t.done ? '2026-07-02' : null; // usado pelo índice de procrastinação
      goTo('inicio');
  ...
  if(page==='eisenhower'){
    // página apenas de leitura/priorização — sem eventos adicionais além dos globais
  }
  ...
  if(page==='materia_detalhe'){
    ...
    if(uz){ uz.addEventListener('click', ()=>{ toast('Arquivo enviado para a pasta da disciplina.', 'upload-cloud'); }); }
    $all('[data-open-material]').forEach(btn=> btn.addEventListener('click', ()=>{
      // simula o aluno abrindo/lendo o material, o que alimentaria o Analytics de Leitura do professor
  ...
  if(page==='avisos'){
    $('#post-announce-btn') && $('#post-announce-btn').addEventListener('click', ()=>{
      ...
      if(!val) return;
      ANNOUNCEMENTS.unshift({author: state.userName||'Professor(a)', subject: SUBJECTS[0].name, text: val, time:'agora', ts: Date.now(), read:false});
      goTo('avisos');
...
function setupQuiz(subId){
  ...
  $all('.quiz-opt').forEach(btn=> btn.addEventListener('click', ()=>{
    if(btn.parentElement.querySelector('.correct, .wrong')) return; // já respondida
...
$('#notif-btn').addEventListener('click', (e)=>{ e.stopPropagation(); $('#notif-panel').classList.toggle('hidden'); });
